...
async def _get(cache: CacheBackend, raw_key: str) -> dict[str, Any] | None:
    try:
...
async def claim(cache: CacheBackend, raw_key: str, body_fingerprint: str, ttl: int) -> Outcome:
    """Atomically claim ``raw_key`` for processing, or classify an existing record.
    ...
    try:
        claimed = await cache.add(_store_key(raw_key), sentinel, ttl)
    except Exception:  # pragma: no cover - fail open on backend error
    ...
    existing = await _get(cache, raw_key)
    if existing is None:
...
async def complete(
    cache: CacheBackend, raw_key: str, body_fingerprint: str, response: dict[str, Any], ttl: int
) -> None:
    ...
    try:
        await cache.set(_store_key(raw_key), record, ttl)
    except Exception:  # pragma: no cover - fail open on backend error
        pass


async def release(cache: CacheBackend, raw_key: str) -> None:
    ...
    try:
        await cache.delete(_store_key(raw_key))
    except Exception:  # pragma: no cover - fail open on backend error
        pass
