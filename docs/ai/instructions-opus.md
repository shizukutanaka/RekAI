...
2. **セマンティックキャッシュが単一閾値 + 線形走査** (`semantic_cache.py`)。
   ...
   +(あれば)NumPy で 60 倍高速化し、残るコストは
   `rekai_semantic_cache_lookup_seconds` で可視化した。閾値では防げない
   「意味を反転させる小編集」(否定・数値) は判別子ガードで弾く。
   **未解決**: 単一閾値そのもの (three-zone 化) と、線形走査を超える探索
   (ANN)。どちらも現規模では過剰と判断して見送っている (O-2)。
...
5. **streaming に Idempotency-Key なし** (意図的・文書化済み — 変更するなら設計から)。
   ~~ストリームには出力レダクションもかからない~~ → **解消**:
   `guardrails.StreamRedactor` がセンチネル接頭辞 (`sk-`/`AKIA`/`-----BEGIN` 等)
   からの分だけ保留する増分レダクション。通常の散文の遅延は 9 文字のみ。
   **バッファ中の領域は絶対にスクラブしないこと** — `{20,}` が最小長でマッチして
   鍵の後半が平文で流出する (回帰テストで固定済み)。
